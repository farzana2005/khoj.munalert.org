

import zunayedHasanSokal from "../assets/zunayed hasan sokal.jpg";
import abdulMazidImg from "../assets/abdul-mazid.jpg";
import abdullahTamim from "../assets/abdullah tamim.jpg";
import fahimMiya from "../assets/fahim miya.jpg";
import hasanImg from "../assets/hasan.jpg";
import ikraIslam from "../assets/ikra islam.jpg";
import khadizaMaya from "../assets/khadiza maya.jpg";
 import mdIbrahim from "../assets/md ibrahim.jpg";
 import siyamAli from "../assets/md siyam ali.jpg";
 import tasimImg from "../assets/md tasim.jpg";
 import mdAymanImg from "../assets/md. Ayman.jpg";
 import mdHossain from "../assets/md. hossain.jpg";
 import mustafizImg from "../assets/mustafiz.jpg";
 import rengnayaMurung from "../assets/rengnaya murung.jpg";
 import rihanHossain from "../assets/rihan hossain.jpg";
 import robiulImg from "../assets/robiul.jpg";
 import samiraImg from "../assets/samira.jpg";
 import zubayerHossain from "../assets/zubayer hossain.jpg";
 import mdTamim from "../assets/md tamim.jpeg";
 



export const memories = [

  {
  name: "জুনায়েদ হোসেন সকাল",
  seat: "চট্টগ্রাম-৯ আসন",
  dates: "February, 2019",
  image_src: {
    small: zunayedHasanSokal,
    large: zunayedHasanSokal,
 link: "junayed-sakal", // <-- manually assign
  },
  story: `ঢাকার লালবাগ থেকে সকাল নামের একটি শিশু জানুয়ারি ২০১৮ থেকে নিখোঁজ।
আজ পেরিয়েছে ৮ বছর, তবুও তাকে উদ্ধার করা যায়নি।
শিশুটি তখন ভিডিও গেম খেলছিল, আর তার ছোট্ট জীবন এক মুহূর্তে হারিয়ে গেল।
পরিবারের প্রতিটি দিন কেটে যাচ্ছে অনিশ্চয়তা, হতাশা এবং ব্যথার সঙ্গে।
কর্তৃপক্ষের সাহায্য প্রার্থনা করে পরিবার বারবার বলছে—আমাদের শিশুটি কোথায়?
এই ৮ বছরের অপেক্ষা, এক পরিবারকে ভেঙে দিয়েছে, কিন্তু আমরা হার মানি না।
আপনি যদি সকালকে দেখে থাকেন বা কোনো তথ্য জানেন, দয়া করে যোগাযোগ করুন এবং সাহায্য করুন।
একটি ছোট্ট সহানুভূতি, একটি শেয়ার, হয়তো তাদের হারানো সন্তানকে ফিরিয়ে আনতে পারে।`

},  
   

{
  name: "আব্দুল মজিদ",
  seat: "আসন:ঢাকা-১৮ ",
  dates: "16 September 2025",
  image_src: {
    small: abdulMazidImg,
    large: abdulMazidImg,
  },
  story: `আমরা প্রত্যেকেই এই পৃথিবীতে বেঁচে থাকতে চাই, এই পৃথিবীর মায়া ত্যাগ করে আমরা কেউ মৃত্যুবরণ করতে চাই না।`
 
},

{
  name: "আবদুল্লাহ তামিম",
   seat: "ঢাকা-১৮ আসন ",
  dates: "16 September 2025",
  image_src: {
    small: abdullahTamim,
    large: abdullahTamim,
  },
  story: `২০ মে ২০২৫। সকাল প্রায় ৯টা। প্রতিদিনের মতো স্কুলে যাওয়ার উদ্দেশ্যে বাড়ি থেকে বেরিয়েছিল তামিম। সরিষা-প্রেমটিয়া মাধ্যমিক বিদ্যালয়ের সপ্তম শ্রেণির এই ছাত্রটি—মো. নজরুল ইসলামের একমাত্র ছেলে—সেদিন আর ফিরে আসেনি। পাংশা উপজেলার কলিমহর ইউনিয়নের সাঁজুরিয়া গ্রামে সেই সকালটাই ছিল শেষ স্বাভাবিক সকাল।
দিন গড়িয়েছে, মাস পেরিয়েছে। তামিম নিখোঁজ হওয়ার প্রায় আট মাস কেটে গেছে। সন্তানের খোঁজে তার পরিবার একা থাকেনি। চ্যানেল ২৪–এর প্রতিবেদন অনুযায়ী, ৩ জুন ২০২৫ তারিখে পাংশা পৌর শহরে পরিবার ও এলাকাবাসী মানববন্ধন কর্মসূচি পালন করেছে। সেই মানববন্ধন ছিল নীরব প্রতিবাদ আর অসহায় আকুতির প্রতিচ্ছবি।
এরপরও থেমে থাকেনি চেষ্টা। ফুটবল ম্যাচে প্ল্যাকার্ড প্রদর্শনের মাধ্যমে, সামাজিক যোগাযোগমাধ্যমে ছবি ও বার্তা ছড়িয়ে দিয়ে তামিমের সন্ধানের আহ্বান জানানো হয়েছে। তবুও জুন ২০২৫–এর পর তার অবস্থান সম্পর্কে কোনো নিশ্চিত সরকারি বা সংবাদমাধ্যমের তথ্য সামনে আসেনি।
তামিম আজও নিখোঁজ। আর তার পরিবার আজও অপেক্ষায়—একটি সকাল, একটি দরজা খোলার শব্দ, কিংবা শুধু একটি খবরের আশায়। যতদিন সে ফিরে না আসে, এই অপেক্ষা শেষ হবে না।?`
},



{
  name: "ফাহিম মিয়া",
   seat: "আসন:ঢাকা-১৮ ",
  dates: "16 September 2025",
  image_src: {
    small: fahimMiya,
    large: fahimMiya,
  },
  story: `আমরা প্রত্যেকেই এই পৃথিবীতে বেঁচে থাকতে চাই, এই পৃথিবীর মায়া ত্যাগ করে আমরা কেউ মৃত্যুবরণ করতে চাই না।`
},

{
  name: "হাসান",
   seat: "আসন:ঢাকা-১৮ ",
  dates: "16 September 2025",
  image_src: {
    small: hasanImg,
    large: hasanImg,
  },
  story: `আমরা প্রত্যেকেই এই পৃথিবীতে বেঁচে থাকতে চাই, এই পৃথিবীর মায়া ত্যাগ করে আমরা কেউ মৃত্যুবরণ করতে চাই না।`
},

{
  name: "ইকরা ইসলাম",
  seat: "আসন:ঢাকা-১৮ ",
  dates: "16 September 2025",
  image_src: {
    small: ikraIslam,
    large: ikraIslam,
  },
  story: `আমরা প্রত্যেকেই এই পৃথিবীতে বেঁচে থাকতে চাই, এই পৃথিবীর মায়া ত্যাগ করে আমরা কেউ মৃত্যুবরণ করতে চাই না।`
},

{
  name: "খাদিজা মায়া",
  seat: "আসন:ঢাকা-১৮ ",
  dates: "16 September 2025",
  image_src: {
    small: khadizaMaya,
    large: khadizaMaya,
  },
  story: `আমরা প্রত্যেকেই এই পৃথিবীতে বেঁচে থাকতে চাই, এই পৃথিবীর মায়া ত্যাগ করে আমরা কেউ মৃত্যুবরণ করতে চাই না।`
},

{
  name: "মোঃ ইব্রাহিম",
  seat: "ঢাকা-১৮ আসন ",
  dates: "16 September 2025",
  image_src: {
    small: mdIbrahim,
    large: mdIbrahim,
  },
  story: `আমরা প্রত্যেকেই এই পৃথিবীতে বেঁচে থাকতে চাই, এই পৃথিবীর মায়া ত্যাগ করে আমরা কেউ মৃত্যুবরণ করতে চাই না।`
},


{
  name: "সিয়াম আলী",
  seat: "ঢাকা-১৮ আসন ",
  dates: "16 September 2025",
  image_src: {
    small:siyamAli ,
    large: siyamAli,
  },
  story: `আমরা প্রত্যেকেই এই পৃথিবীতে বেঁচে থাকতে চাই, এই পৃথিবীর মায়া ত্যাগ করে আমরা কেউ মৃত্যুবরণ করতে চাই না।`
},

{
  name: "মোঃ তাসিম",
  seat: "ঢাকা-১৮ আসন ",
  dates: "16 September 2025",
  image_src: {
    small: tasimImg,
    large: tasimImg,
  },
  story: `আমরা প্রত্যেকেই এই পৃথিবীতে বেঁচে থাকতে চাই, এই পৃথিবীর মায়া ত্যাগ করে আমরা কেউ মৃত্যুবরণ করতে চাই না।`
},

{
  name: "মোঃ আয়মান",
  seat: "ঢাকা-১৮ আসন ",
  dates: "16 September 2025",
  image_src: {
    small: mdAymanImg,
    large: mdAymanImg,
  },
  story: `আমরা প্রত্যেকেই এই পৃথিবীতে বেঁচে থাকতে চাই, এই পৃথিবীর মায়া ত্যাগ করে আমরা কেউ মৃত্যুবরণ করতে চাই না।`
},

{
  name: "মোঃ হোসেন",
  seat: "ঢাকা-১৮ আসন ",
  dates: "16 September 2025",
  image_src: {
    small: mdHossain,
    large: mdHossain,
  },
  story: `আমরা প্রত্যেকেই এই পৃথিবীতে বেঁচে থাকতে চাই, এই পৃথিবীর মায়া ত্যাগ করে আমরা কেউ মৃত্যুবরণ করতে চাই না।`
},

{
  name: "মুস্তাফিজ",
  seat: "ঢাকা-১৮ আসন ",
  dates: "16 September 2025",
  image_src: {
    small:mustafizImg,
    large: mustafizImg,
  },
  story: `আমরা প্রত্যেকেই এই পৃথিবীতে বেঁচে থাকতে চাই, এই পৃথিবীর মায়া ত্যাগ করে আমরা কেউ মৃত্যুবরণ করতে চাই না।`
},

{
  name: "রেংনায়া মুরুং",
  seat: "ঢাকা-১৮ আসন ",
  dates: "16 September 2025",
  image_src: {
    small: rengnayaMurung,
    large: rengnayaMurung,
  },
  story: `আমরা প্রত্যেকেই এই পৃথিবীতে বেঁচে থাকতে চাই, এই পৃথিবীর মায়া ত্যাগ করে আমরা কেউ মৃত্যুবরণ করতে চাই না।`
},

{
  name: "রিহান হোসেন",
  seat: "ঢাকা-১৮ আসন ",
  dates: "16 September 2025",
  image_src: {
    small: rihanHossain,
    large: rihanHossain,
  },
  story: `আমরা প্রত্যেকেই এই পৃথিবীতে বেঁচে থাকতে চাই, এই পৃথিবীর মায়া ত্যাগ করে আমরা কেউ মৃত্যুবরণ করতে চাই না।`
},

{
  name: "রবিউল",
  seat: "ঢাকা-১৩ আসন ",
  dates: "December, 2025",
  image_src: {
    small: robiulImg,
    large: robiulImg,
  },
  story: `আমরা প্রত্যেকেই এই পৃথিবীতে বেঁচে থাকতে চাই, এই পৃথিবীর মায়া ত্যাগ করে আমরা কেউ মৃত্যুবরণ করতে চাই না।`
},

{
  name: "সামিরা",
  seat: "খুলনা-৬ আসন",
  dates: "August, 2025",
  image_src: {
    small: samiraImg,
    large: samiraImg,
  },
  story: `আমরা প্রত্যেকেই এই পৃথিবীতে বেঁচে থাকতে চাই, এই পৃথিবীর মায়া ত্যাগ করে আমরা কেউ মৃত্যুবরণ করতে চাই না।`
},

{
  name: "জুবায়ের হোসেন",
  seat: "ঢাকা-১৮ আসন ",
  dates: "16 September 2025",
  image_src: {
    small: zubayerHossain,
    large: zubayerHossain,
  },
  story: `আমরা প্রত্যেকেই এই পৃথিবীতে বেঁচে থাকতে চাই, এই পৃথিবীর মায়া ত্যাগ করে আমরা কেউ মৃত্যুবরণ করতে চাই না।`
},


{
  name: "মোঃ তামিম",
  seat: "ঢাকা-১৮ আসন ",
  dates: "16 September 2025",
  image_src: {
    small: mdTamim,
    large: mdTamim,
  },
  story: `তামিম ছিল খুব সাধারণ একটি ছেলে, পরিবারের সকলের চোখের মণি। তার হাসিতে ভরে থাকত বাড়ি। কিন্তু হঠাৎ করেই সব কিছু বদলে গেল। গত ২৪-০১-২০২৬ তারিখে রাত ৯টার দিকে সে বাসা থেকে নতুন বাজার আসার পথে হঠাৎ নিখোঁজ হয়ে যায়। অনেক খোঁজাখুঁজি করেও তার কোনো সন্ধান মেলেনি।
নিখোঁজ শিশুটির মাথায় একটি কাটার দাগ রয়েছে। পরনে ছিল প্যান্ট, শার্ট এবং একটি কালো সুয়েটার।
যদি কোনো হৃদয়বান ব্যক্তি এই শিশুটির খোঁজ জানেন, দয়া করে জানাবেন। একটি অসহায় পরিবার তাদের সন্তানের অপেক্ষায় প্রতিটি মুহূর্ত কষ্ট সহ্য করছে। আপনার সামান্য তথ্য তাদের বড় সাহায্য করতে পারে।
আমরা কি এই বার্তাটি আরও মানুষের কাছে পৌঁছে দিতে শেয়ার করতে পারি?`
},




]